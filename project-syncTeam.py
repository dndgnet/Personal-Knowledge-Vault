...
print(
    f"{myTerminal.INFORMATION}Sync Project Content with a shared team folder{myTerminal.RESET}\n"
)
print("")
...
synchFolderPath = projectConfig.get("PrivateShareFolder", "")
    ...
    if not os.path.exists(targetNotePath):
        ...
        if os.path.getmtime(note.filePath) > os.path.getmtime(targetNotePath):
            ...
            try:
                ...
                syncNoteAttachments(note)
                log_sync_action(
                    "UPDATE_NOTE", f"Updated note '{note.fileName}' in sync folder"
                )
                print(
                    f"{myTerminal.SUCCESS}Updated note '{note.fileName}' in sync folder.{myTerminal.RESET}"
                )
                sendCount += 1
            except Exception as e:
                print(
                    f"{myTerminal.ERROR}Failed to update note '{note.fileName}' in sync folder: {e}{myTerminal.RESET}"
                )
                log_sync_action(
                    "UPDATE_NOTE_FAILED",
                    f"Failed to update note '{note.fileName}' in sync folder: {e}",
                )

print(f"Send process completed. {sendCount} notes sent to sync folder.")

if not myInputs.ask_yes_no_from_user(
    "Do you want retrieve any files from the sync folder (for example, copy your team's notes locally)?"
):
...
if len(remoteNotes) == 0:
    print(
        f"{myTerminal.INFORMATION}No notes found in sync folder to retrieve.{myTerminal.RESET}"
    )
    exit(0)
...
for remoteNote in remoteNotes:
    ...
    localNotePath = os.path.join(
        myPreferences.root_projects(),
        selectedProject,
        remoteNote.fileName.replace(".md", f"_{remoteNote.author}.md"),
    )
    needsToBeRetrieved = False
    if not os.path.exists(localNotePath):
        ...
        if os.path.getmtime(
            os.path.join(synchFolderPath, remoteNote.fileName)
        ) > os.path.getmtime(localNotePath):
            needsToBeRetrieved = True
    ...
    if needsToBeRetrieved:
        ...
        try:
            shutil.copy2(
                os.path.join(synchFolderPath, remoteNote.fileName), localNotePath
            )
            log_sync_action(
                "UPDATE_REMOTE_NOTE",
                f"Updated local note '{remoteNote.fileName}' from sync folder",
            )
            syncRemoteNoteAttachments(remoteNote)
            print(
                f"{myTerminal.SUCCESS}Updated local note '{remoteNote.fileName}' from sync folder.{myTerminal.RESET}"
            )
            retrieveCount += 1
        except Exception as e:
            print(
                f"{myTerminal.ERROR}Failed to update local note '{remoteNote.fileName}' from sync folder: {e}{myTerminal.RESET}"
            )
            log_sync_action(
                "UPDATE_REMOTE_NOTE_FAILED",
                f"Failed to update local note '{remoteNote.fileName}' from sync folder: {e}",
            )
