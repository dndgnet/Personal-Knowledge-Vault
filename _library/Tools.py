...
from .Notes import NoteData, get_note_tags, get_Notes_as_list
    ...
    for date_format in _datetime_formats:
        try:
            ...
            if re.match(r"^\d{1,2}:\d{2}$", date_string):
                date_string = (
                    f"{datetime.datetime.now().strftime('%Y-%m-%d')} {date_string}"
                )
...
def generate_unique_identifier(timestamp_id, noteType, title) -> str:
    ...
    titleLettersAndNumbers = letters_and_numbers_only(
        title, maxLength=200
    ...
    while not is_NewNote_identifier_unique(uniqueIdentifier):
        ...
        print(
            f"\t{myTerminal.WARNING}Note identifier '{uniqueIdentifier}' already exists. Generating a new one...{myTerminal.RESET}"
        )
        selectedDateTime = datetime.datetime.strptime(
            timestamp_id, myPreferences.timestamp_id_format()
        )
        timestamp_id = (selectedDateTime + datetime.timedelta(seconds=1)).strftime(
            myPreferences.timestamp_id_format()
        )
        uniqueIdentifier = f"{timestamp_id}_{noteType}_{titleLettersAndNumbers}"
...
def generate_tag_from_projectName(projectName: str) -> str:
    ...
    tag = "p_" + letters_and_numbers_only(projectName).replace(" ", "_").replace(
        "&", "and"
    )
...
def decimal_from_string(value: str) -> Decimal:
    ...
    value = (
        ...
        .replace("%", "")
        .strip()
    )  # Remove commas and dollar signs, and strip whitespace
