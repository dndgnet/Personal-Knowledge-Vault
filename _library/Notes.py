...
def write_Note_to_path(notePathAndFile: str, noteContent: str) -> bool:
    ...
    try:
        ...
        print(
            f"{myTerminal.ERROR}Failed to write to note file '{notePathAndFile}': {e}{myTerminal.RESET}"
        )
        return False


def replace_text_between_tags(
    areaName: str, noteBody: str, newContent: str
) -> tuple[bool, str]:
    """
    ...
    if startTag in noteBody and endTag in noteBody:
        ...
        print(
            f"{myTerminal.WARNING}Area tags not found for '{areaName}'.{myTerminal.RESET}"
        )
        return False, ""
...
def get_Note_from_path(notePath: str, noteFileName: str) -> NoteData:
    ...
    if uniqueIdentifier == "":
        uniqueIdentifier = noteFileName.split(".")[0].split("_")[
            0
        ]  # Use the file name without extension as the unique identifier
    ...
    private = (
        True
        if get_stringValue_from_frontMatter("private", frontMatter).lower()
    ...
    for actionItem in re.findall(
        ...
        if match:
            ...
            nextLine = (
                remaining_text.splitlines()[1].strip()
...
def get_Notes_as_list(
    target_dir: str, includePrivateNotes=True, includeArchivedProjects=True
) -> list[NoteData]:
    """
...
def sort_Notes_by_date(
    notes: list[NoteData], descending: bool = True
) -> list[NoteData]:
    """
...
def get_tags_from_Text(note: str) -> list:
    ...
    frontMatterTagsString = (
        frontMatterTags_match.group(1).strip() if frontMatterTags_match else ""
    )
...
def get_Note_Last_Project_Note_ByType(
    projectName: str, noteType: str
) -> tuple[bool, NoteData]:
    """
    ...
    if projectName == "" or projectName is None:
        ...
        allNotes = get_Notes_as_list(
            os.path.join(myPreferences.root_projects(), projectName), True
        )

    sortedNotes = sorted(
        allNotes, key=lambda note: (note.project, note.date), reverse=True
    )
    ...
    for note in sortedNotes:
        if (
            note.project.upper() == projectName.upper()
            and noteType.upper() in note.type.upper()
        ):
...
def load_MostRecentProjectProgressNote(projectName: str) -> tuple[bool, NoteData]:
    ...
    for note in sortedNotes:
        if (
            note.project.upper() == projectName.upper()
            and "progress".upper() in note.type.upper()
        ):
            return True, note  # Return the first matching note
...
def clone_note(sourceNotePath) -> str:
    ...
    if not os.path.isfile(sourceNotePath):
        print(
            f"{myTerminal.ERROR}Source note '{sourceNotePath}' does not exist.{myTerminal.RESET}"
        )
        return ""
    ...
    newFrontMatter = re.sub(
        r"id:\s*.*", f"id: {timestamp_id}", newFrontMatter
    )  # Update the id in front matter
    newFrontMatter = re.sub(
        r"created:\s*.*", f"created: {timestamp_full}", newFrontMatter
    )  # Update the created in front matter
    newFrontMatter = re.sub(
...
def replace_lineLabelValue_in_noteBody(
    lineLabel: str, newValue: str, noteBody: str
) -> str:
    """
...
def get_listValue_from_frontMatter(valuePrefix: str, frontMatter: str) -> list:
    ...
    if keywords_match:
        ...
        return [
            keyword.strip() for keyword in keywords_string.split(",") if keyword.strip()
        ]
...
def get_stringValue_from_noteBodyForTableCell(valueLabel: str, noteBody: str) -> str:
    ...
    return (
        get_stringValue_from_noteBody(valueLabel, noteBody)
        .replace("|", "")
        .replace("\n", "<br/>")
        .strip()
    )
...
def get_sectionValue_from_noteBodyForTableCell(valueLabel: str, noteBody: str) -> str:
    ...
    return (
        get_sectionValue_from_noteBody(valueLabel, noteBody)
        .replace("|", "")
        .replace("\n", "<br/>")
        .strip()
    )
