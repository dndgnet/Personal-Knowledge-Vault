...
def main():
    ...
    if selectedProjectName is None or selectedProjectName == "":
        selectedProjectName = ""
        templates, selectedTemplateName, selectedTemplateIndex = (
            myInputs.select_template("pkv")
        )
    else:
        templates, selectedTemplateName, selectedTemplateIndex = (
            myInputs.select_template("project")
        )
    ...
    note_Content = myInputs.get_templateMerge_Values_From_User(
        timestamp_id,
        timestamp_date,
        timestamp_full,
        selectedProjectName,
    ...
    if selectedProjectName != "" and selectedProjectName is not None:
        ...
        if frontMatterTags is not None and frontMatterTags != "":
            projectTag = (
                myTools.generate_tag_from_projectName(selectedProjectName) + ", "
            )
        else:
        ...
        for journal in notes:
            if (
                ...
                if addToJournal:
                    ...
                    if myPreferences.include_backlinks_to_DailyJournal():
