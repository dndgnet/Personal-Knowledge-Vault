...
hubNoteFound, hubNote = myNotes.get_Note_Last_Project_Note_ByType(
    selectedProject, "project-hub"
)

if not hubNoteFound:
    print(
        f"{myTerminal.WARNING}No hub note found for project '{selectedProject}'.{myTerminal.RESET}"
    )
    exit(1)
...
if success:
    ...
    print(
        f"{myTerminal.WARNING}BurnDown tags not found in hub note for project '{selectedProject}'.{myTerminal.RESET}"
    )
...
success, newNoteBody = myNotes.replace_text_between_tags(
    "Assumptions", hubNote.noteBody, assumptionsContent
)
if success:
    hubNote.noteBody = newNoteBody
    myNotes.update_NoteBody(hubNote, newNoteBody)
    ...
    print(
        f"{myTerminal.WARNING}Assumptions tags not found in hub note for project '{selectedProject}'.{myTerminal.RESET}"
    )

# deal with issues
issuesContent = myProjects.raid_Issues(
    selectedProject, allNotesForProject, returnTableFormat=True
)
success, newNoteBody = myNotes.replace_text_between_tags(
    "Issues", hubNote.noteBody, issuesContent
)
if success:
    hubNote.noteBody = newNoteBody
    myNotes.update_NoteBody(hubNote, newNoteBody)
    ...
    print(
        f"{myTerminal.WARNING}Issues tags not found in hub note for project '{selectedProject}'.{myTerminal.RESET}"
    )
...
if success:
    ...
    print(
        f"{myTerminal.WARNING}Change Requests tags not found in hub note for project '{selectedProject}'.{myTerminal.RESET}"
    )

# deal supporting documentation
supportingDocumentation = myProjects.notePart_SupportingDocumentation(
    selectedProject, allNotesForProject, returnTableFormat=False
)

success, newNoteBody = myNotes.replace_text_between_tags(
    "Supporting_Documentation", hubNote.noteBody, supportingDocumentation
)
...
for sectionNoteType in myProjects.projectNoteTypesWhereThereCanBeOnlyOne:
    ...
    sectionNoteFount, sectionNote = myNotes.get_Note_Last_Project_Note_ByType(
        selectedProject, sectionNoteType
    )
    if sectionNoteFount:
        success, newNoteBody = myNotes.replace_text_between_tags(
            sectionNoteType, hubNote.noteBody, sectionNote.noteBody
        )
